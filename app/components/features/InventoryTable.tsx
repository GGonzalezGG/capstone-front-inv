...
const statusConfig: Record<InventoryItemStatus, { variant: 'success' | 'warning' | 'danger', text: string }> = {
...
const InventoryTable: React.FC<InventoryTableProps> = ({ 
  ...
  const columns: ColumnDef<InventoryItem>[] = [
    ...
    },
    {
      ...
      accessor: 'quantity',
      render: (item) => (
        <span className={`${item.status === 'low_stock' ? 'text-yellow-600 font-semibold' : ''} ${item.status === 'expired' ? 'text-red-600 font-semibold' : ''}`}>
          {item.quantity} uds.
        </span>
      )
    },
